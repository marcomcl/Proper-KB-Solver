...
public class Term implements VariableContainer {
    ...
    public boolean isStandard() {
    	return arity() == 0 && function.charAt(0) == 'n' && isInteger(function.substring(1));
    }
    
    public boolean isNull() {
    	return arity() == 0 && function.length() > 4 && function.substring(0, 4).equals("null") && isInteger(function.substring(4));
    }
    ...
    public void substitute(Term term, Term forVar) {
        if (isVariable() &&
                this.equals(forVar) || isNull() && this.equals(forVar) || isConstant() && this.equals(forVar)) {
        ...
        for (int i = 0; i < innerTerms.size(); i++) {
            ...
            if (!innerTerm.isVariable() || !innerTerm.isNull() || !isConstant()) {
    ...
    public Set<Term> standardNames() {
        ...
        for (Term innerTerm : innerTerms) {
            if (innerTerm.isStandard()) {
                stdNames.add(innerTerm);
            } else {
                stdNames.addAll(innerTerm.standardNames());
            }
        }
    ...
    public Set<Term> constants() {
        ...
        for (Term innerTerm : innerTerms) {
            if (innerTerm.isConstant() && !innerTerm.isNull()) {
                constants.add(innerTerm);
            } else {
                constants.addAll(innerTerm.constants());
            }
        }
